...
function BigDebuffs:MakePlayerName (name) --{{{
    if not name then name = "NONAME" end
    return "|Hplayer:" .. name .. "|h" .. (name):upper() .. "|h";
end --}}}

function BigDebuffs:ColorText (text, color) --{{{
...
BigDebuffs_C.ClassesColors = { };

local LC = _G.LOCALIZED_CLASS_NAMES_MALE;

function BigDebuffs:GetClassColor (englishClass) -- {{{
    if not BigDebuffs_C.ClassesColors[englishClass] then
        if RAID_CLASS_COLORS and RAID_CLASS_COLORS[englishClass] then
            BigDebuffs_C.ClassesColors[englishClass] = { RAID_CLASS_COLORS[englishClass].r, RAID_CLASS_COLORS[englishClass].g, RAID_CLASS_COLORS[englishClass].b };
        else
            BigDebuffs_C.ClassesColors[englishClass] = { 0.63, 0.63, 0.63 };
        end
    end
    return unpack(BigDebuffs_C.ClassesColors[englishClass]);
end -- }}}

BigDebuffs_C.HexClassColor = { };

function BigDebuffs:GetClassHexColor(englishClass) -- {{{

    if not BigDebuffs_C.HexClassColor[englishClass] then

        local r, g, b = self:GetClassColor(englishClass);

        BigDebuffs_C.HexClassColor[englishClass] = ("FF%02x%02x%02x"):format( r * 255, g * 255, b * 255);

    end

    return BigDebuffs_C.HexClassColor[englishClass];
end -- }}}

function BigDebuffs:CreateClassColorTables () -- {{{
    if RAID_CLASS_COLORS then
        ...
        for class in pairs(RAID_CLASS_COLORS) do
            if LC[class] then -- thank to a wonderful add-on that adds the wrong translation "Death Knight" to the global RAID_CLASS_COLORS....
                BigDebuffs:GetClassHexColor(class);
            else
                RAID_CLASS_COLORS[class] = nil; -- Eat that!
                print("BigDebuffs: |cFFFF0000Stupid value found in _G.RAID_CLASS_COLORS table|r\nThis will cause many issues (tainting), BigDebuffs will display this message until the culprit add-on is fixed or removed, the Stupid value is: '", class, "'");
            end
        end
    else
        BigDebuffs:Debug(ERROR, "global RAID_CLASS_COLORS does not exist...");
    end
end -- }}}
-- }}}

function BigDebuffs:Error(message)
    UIErrorsFrame:AddMessage("BigDebuffs: " .. message, 1, 0, 0, 1, UIERRORS_HOLD_TIME);
    self:Print(BigDebuffs:ColorText(message, 'FFFF3030'));
    return message;
end

-- function BigDebuffs:UnitName(Unit) {{{
...
-- function BigDebuffs:RotateTexture(self, degrees) {{{
...
do
    ...
    function BigDebuffs:Debug(...)
        if not BigDebuffs.db.global.Debug then return end;
...
-- function BigDebuffs:GetOPtionPath(info) {{{
function BigDebuffs:GetOPtionPath(info)
...
function BigDebuffs:pairs_ordered (t, reverse, SortKey) -- Not to be used where performance and memory are important
